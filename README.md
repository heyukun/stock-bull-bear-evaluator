...
- **前置筛选关卡**：全量评估前五道关卡（高权重因子利空筛查 + 当前/长期整体评分 ≥7 分），任一触发即终止流程。
- **因子 13 业绩动能**：基于多季度财务趋势的 14 项检查直接入分，并强制与卖方一致预期做 13a 预期差校验。
- **因子 14 估值分类型判定**：先判定公司类型（成长/价值/周期）再选主估值指标（成长→PEG/PS，价值→PE/股息率，周期→PB）。
- **尾部风险熔断**：偿债/审计/质押/客户集中度四项红线，触发任一项综合评级封顶「看空」。
...
8. 因子 13 必须执行 13a 预期差校验；因子 14 估值必须先判定公司类型再选主估值指标。
9. 前置筛选关卡一票否决，任一触发即终止全部流程。
